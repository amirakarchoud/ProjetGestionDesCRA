...
function Banner({mode = '', text= '', week = []}) {
        ...
        <div className={styles.list}>
          {(week).map((day, i) => (
            <div key={i} className={styles.element}>
              ...
              <div className={styles.date}>{day.format(DateTimeFormatter.ofPattern('(dd/MM/yyyy)'))}</div>
            </div>
          ))}
        </div>
      </>
  ...
  if (mode === 'button') {
    return <div><Button color="primary" variant="contained">{text}</Button></div>;
  }
  return <div>Warning: No mode provided</div>;
}

function TableBanner({ mode, week, text }) {
  return (
    <Stack direction="row" className={`${styles.block} ${mode === 'button' ? '' : styles.grey}`}>
